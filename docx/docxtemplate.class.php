...
class DOCXTemplate {
				...
				foreach( $m[0] as $v ) {
					...
					if (isset( $this->data[ $var ] )) {
						if (is_scalar( $this->data[ $var ] ))
							$doc = str_replace( $v, $this->_esc( utf8_encode($this->data[ $var ]) ), $doc );
					} else {
	...
	private function _unzip( $filename, $is_data = false ) {
		...
		if ($is_data) {
			...
			$this->package['filename'] = 'default.docx';
...
//		$fh = fopen( $filename, 'wb' );
	...
	function downloadAs( $filename, $exit = true ) {
		...
		header('Content-type: application/vnd.openxmlformats-officedocument.wordprocessingml.documen');
